...
A secondmate's idle endpoint is healthy, and parent supervision relies on its routed status rather than treating a quiet pane as stale; even a secondmate's declared `paused:` wait is tracked through that routed status and its own home's backlog, never through pane-based rechecks.
