...
A crew whose current state reads `awaiting-validation-trigger` has committed its implementation and is idle waiting for exactly that trigger, so it is your pending action and never a finished task.
