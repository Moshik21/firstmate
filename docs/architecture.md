...
In that status-log fallback, an idle `no-mistakes` ship whose last event is its implementation `done:` reports the distinct `awaiting-validation-trigger` state, which names firstmate's own unstarted validation trigger instead of reading as finished work.
That verdict requires both a completed no-mistakes lookup that attributes no active or terminal run to the crew's current code and no PR recorded in either task metadata or the crew's own status stream, so an unavailable lookup, a direct-PR or local-only ship, a scout, a still-working or dead worker, a validating or terminal run, and an already recorded PR are all excluded from it.
`bin/fm-pr-lib.sh` owns the one recorded-PR reader behind that check, shared with `bin/fm-fleet-snapshot.sh`, so a GitLab merge request counts as a recorded PR exactly as a GitHub pull request does.
In that same status-log fallback, a declared external wait reports the distinct `paused` state with its reason.
...
Remote placement is independent of the remote home's ordinary local session backend, and individual workers are never placed remotely by this feature.
