...
A declared `paused:` or `captain-held` window whose endpoint no longer captures at all follows that same surface-once-then-recheck path instead of dropping out of supervision silently, again only on a confidently dead agent; a transient capture failure with a live or unreadable agent stays the historical silent skip.
Live or inconclusive liveness remains fail-open at that initial surface, and the secondmate idle-endpoint exemption is unconditional: the stale loop skips a secondmate window outright, including one declaring `paused:`, because parent supervision relies on its routed status.
...
[`watcher-continuity.md`](watcher-continuity.md#arm-layer-cycle-contract) owns the arm layer's successor, terminal-delivery, and typed clean-close failure contract, and the `--restart` reap bound.
