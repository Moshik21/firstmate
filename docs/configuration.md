...
The session-start digest additionally runs `bin/fm-crew-fitness.sh` for every live endpoint, which needs a live pane's argv, environment, or foreground working directory; only tmux and herdr can report any of those, so a task on zellij, Orca, or cmux always reads `unknown` there, and the tmux and herdr guides' limits own their remaining blind spots.
