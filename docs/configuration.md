...
For claude crewmates and scouts, a task's lifecycle hooks belong to that task's own worktree, so `fm-spawn.sh` refuses the launch before arming the busy-state contract when the resolved worktree is a firstmate home or when its `.claude` directory or existing `.claude/settings.local.json` is a symlink or other redirected path.
Teardown deletes that per-task `.claude/settings.local.json` only from a disposable task worktree, never from a firstmate home, and then runs [`bin/fm-claude-task-hook-cleanup.sh`](../bin/fm-claude-task-hook-cleanup.sh) over this home's own local settings.
That sweep removes only Firstmate task hooks that name this home's state directory and a task id with no surviving `state/<id>.meta`, so the primary's own turn-end guards and any unrelated hooks stay; unsafe or malformed settings are reported and left untouched.
